...
export type AiVaultSessionProject = {
  // 'structured' aggregates every workspace under one structured-project root
  // into a single project scope. The renderer computes it; /shared only consumes
  // the resulting descriptor, so it never needs the aggregation logic itself.
